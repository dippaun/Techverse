...
$stmt = $pdo->prepare(
    'INSERT INTO students
        (full_name, email, phone, address, enrollment_no, course, arena, team_members, payment_id, team_id, password, status, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "pending", NOW())'
);

try {
    $stmt->execute([
        $fullName, $email, $phone, $address, $enrollmentNo, $course,
...
jsonResponse([
    ...
    'student' => [
        ...
        'enrollment_no' => $enrollmentNo, 'course' => $course, 'arena' => $arena,
        'team_members' => json_decode($teamMembersJson, true), 'payment_id' => $paymentId,
